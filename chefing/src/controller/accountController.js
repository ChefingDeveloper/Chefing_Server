...
exports.login = async(req, res) => {
 ...
 let insertUserResult
  ...
  try {
     ...
     console.log('user_age: '+data.user_age)
     console.log('user_email: '+data.user_email)
     console.log('token: '+jwt.sign(data.user_email, data.user_id));
     console.log("==============================");
     ...
     if(chkToken != undefined){ // 토큰이 이미 있는 경우 (로그인 되어있는 경우)
      ...
      if(chkToken.user_email == user_email){
        console.log("성공적으로 로그인 되었습니다");
        token = jwt.sign(data.user_email, data.user_id);
        console.log(token);
        res.status(200).send({
            result : {
            message : "success",
      ...
      checkEmailResult = await accountModel.checkEmail(connection, data.user_email)
        if(checkEmailResult.length != 0){ // 기기를 변경했을 경우
            console.log("다른기기에서 접속했습니다");
            token = jwt.sign(data.user_email, checkEmailResult[0].user_id);
            res.status(200).send({
                "result" : {
          ...
          insertUserResult = await accountModel.inserUserData(connection, data)
            ...
            //     return next("500");
            //   }
            token = jwt.sign(data.user_email, insertUserResult[0].user_id);
